...
class Trainer(object):
    ...
    def step_ema(self): #actualiza el ema
    ...
    def save(self, epoch):
        ...
        print(f'[ utils/training ] Saved model to {savepath}', flush=True)  # flush es para salvar el buffer del os en el disco ver si esto srive en caso de que se apague el pc

    def load(self, epoch):
        '''
            loads model and ema from disk
        '''
        loadpath = os.path.join(self.logdir, f'state_{epoch}.pt')  # probablemente haya que arreglar esto... parece que funciona bien...
    ...
    def render_samples(self, batch_size=2, n_samples=2):   # clase render
        '''
        ...
        for i in range(batch_size):
         ...
         #   self.renderer.composite(savepath, observations)                       ### renderer
